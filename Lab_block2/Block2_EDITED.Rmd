...
In our assignment, we have followed the following key stages:

1)Installing "tree" and "rpart" packages.

2)Reading and adjusting selected adult data set and partition into training/validation/test as 40/30/30.

3)Building decision tree models with three different settings in which are fitted to the training data:
1- Decision Tree with default settings (denoted as "tree_a")
2- Decision Tree with smallest allowed node size equal to 5000(denoted as "tree_b")
3- Decision tree with a minimum deviance to 0.0005(denoted as "tree_c") 

4)Calculating the "Misclassification rate and Accuracy" of each decision tree for the training and
validation sets.

5)Selecting the tree model that needs to be optimized. We select the decision tree with a minimum deviance to 0.0005 ("tree_c") due to overfitting.

6)Using training and validation sets to choose the optimal tree depth in the model "tree_c": study the trees up to 50 leaves.

. 
Use  Present a graph of the dependence
